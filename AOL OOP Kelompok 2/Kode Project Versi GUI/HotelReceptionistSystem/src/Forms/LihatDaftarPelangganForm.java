...
public class LihatDaftarPelangganForm {
	...
	private void SetupTableDaftarPelanggan() {
		...
		tableDaftarPelanggan.setFillsViewportHeight(true);
		...
		tableDaftarPelanggan.setAutoCreateRowSorter(true);
	...
	private void LoadDataTableDaftarPelanggan() {
		...
		for (Transaction transaction : transactionsData) {
			tableModel.addRow(new Object[] { number, transaction.getCustomer().getName(), transaction.getRoomNumber(), DateHelper.formatDate(transaction.getCheckInDate(), "dd-MM-yyyy"), transaction.getLengthOfStay() + " hari" });
			number++;
		}
